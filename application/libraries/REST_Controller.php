...
abstract class REST_Controller extends \CI_Controller {
    ...
    protected function CekToken(){
        return $this->db->where('token', '1')->get('ms_token')->num_rows();
    }
    protected function decodeToken($token) {
        $data = $this->db->where('access_token',$token)->get('ms_token')->row_object();
        if ($data) {
            return JWT::decode($token,'paujiah',array('HS256'));
        } else {
            return 0;
        }
    }
